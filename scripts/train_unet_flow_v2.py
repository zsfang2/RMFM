...
from rmfm.paths import DEFAULT_DATASET_ROOT, DEFAULT_UNET_FLOW_V2_CHECKPOINT_DIR  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output_dir", type=Path, default=DEFAULT_UNET_FLOW_V2_CHECKPOINT_DIR)
    ...
    # Source channel dropout
    parser.add_argument("--source_dropout", type=float, default=0.5,
        help="Probability of zeroing the source channel in condition during training (0=always keep, 1=always drop).")
    ...
    parser.add_argument("--device", type=str, default="cuda",
        help="Training device, e.g. cuda, cuda:0, cuda:1, or cpu.")
    add_gpu_argument(parser)
    parser.add_argument("--channels_last", action="store_true", help="Use NHWC memory format.")
    parser.add_argument("--allow_tf32", action="store_true", help="Enable TF32 on Ampere+ GPUs.")
...
def apply_source_dropout(condition: torch.Tensor, source_dropout: float, training: bool) -> torch.Tensor:
    """Randomly zero the source channel (index 1) per sample during training."""
    if not training or source_dropout <= 0.0:
        return condition
    cond = condition.clone()
    mask = torch.rand(cond.shape[0], device=cond.device) < source_dropout
    cond[mask, 1] = 0.0
    return cond
...
def flow_matching_loss(
    ...
    condition = apply_source_dropout(condition, source_dropout, training=True)
...
def evaluate_loss(
    ...
    """Evaluate validation loss. Pass source_dropout=1.0 to test no-source performance."""
    model.eval()
    losses = []
    for batch_idx, batch in enumerate(loader):
        ...
        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
            loss = flow_matching_loss(model, image, condition, source_dropout=source_dropout)
...
def main() -> None:
    ...
    print(f"Source dropout: {args.source_dropout}")
    train_model = model
    if args.compile:
    ...
    while step < args.max_steps:
        for batch in train_loader:
            ...
            if step % args.val_every == 0:
                ...
                # Validate with source dropped (mimics inference scenario)
                ...
                if is_best:
                    ...
                    best_step = step
                print(f"\n[step {step}] val_loss(no_source)={val_loss:.6f}")
                checkpoint_extra = {
                    "val_loss": val_loss,
