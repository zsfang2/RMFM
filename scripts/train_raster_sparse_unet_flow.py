...
from rmfm.paths import DEFAULT_DATASET_ROOT, DEFAULT_RASTER_SPARSE_UNET_FLOW_CHECKPOINT_DIR  # noqa: E402
from rmfm.sparse import build_sparse_raster_condition, parse_counts  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train C1 Raster-Conditioned FM-UNet with building + sparse map + mask.")
    parser.add_argument("--dataset_root", type=Path, default=DEFAULT_DATASET_ROOT)
    parser.add_argument("--output_dir", type=Path, default=DEFAULT_RASTER_SPARSE_UNET_FLOW_CHECKPOINT_DIR)
    ...
    parser.add_argument(
        ...
        type=str,
        default="5,9,25",
        help="Comma-separated exact sparse sample counts used dynamically during training.",
    )
...
def flow_matching_loss(
    ...
    condition, _, _ = build_sparse_raster_condition(
        image=image,
...
def evaluate_loss(
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=amp_enabled):
            loss = flow_matching_loss(model, image, condition, sparse_counts=sparse_counts, fixed_count=fixed_count)
...
def main() -> None:
    args = parse_args()
    sparse_counts = parse_counts(args.sparse_counts)
    ...
    print(f"Sparse counts: {sparse_counts}")
    train_model = model
    if args.compile:
    ...
    while step < args.max_steps:
        for batch in train_loader:
            ...
            if step % args.val_every == 0:
                ...
                val_losses = []
                for sparse_count in sparse_counts:
                    val_losses.append(
                        evaluate_loss(
                            train_model,
                            val_loader,
                ...
                if is_best:
                    ...
                    best_step = step
                print(f"\n[step {step}] val_loss(c1_mean)={val_loss:.6f}")
                checkpoint_extra = {
                    "val_loss": val_loss,
